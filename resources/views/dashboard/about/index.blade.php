        ...
        <div class="col-md-7 align-self-center text-right">
            <div class="d-flex justify-content-end align-items-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">All Aboutus </li>
                </ol>
                <a href="{{ route('about.create') }}"> <button type="button" class="btn btn-info d-none d-lg-block m-l-15"><i
                    class="fa fa-plus-circle"></i> Create Aboutus</button> </a>
            </div>
        </div>
    </div>
                            ...
                            <tbody>
                                @forelse ($abouts as $index => $item)
                                    <tr>
                                        ...
                                        <td>
                                            {{-- <a href="/about/{{ $item->id }}/edit"><i class="icon-note"></i></a>
                                            <a href="#"><button type="button" class="btn btn-danger" onclick="ConfirmDelete({{ $item->id }})">Delete</button></a> --}}
                                            <a href="{{ route('about.edit',$item->id) }}" class="btn btn-rounded btn-xs btn-info m-r-5" data-toggle="tooltip" data-original-title="Edit">
                                                ...
                                                <form id="delete-form-{{ $item->id }}" action="{{route('about.destroy',$item->id)}}" method="post">
                                                @csrf
                                                @method('delete')
                                                </form>
