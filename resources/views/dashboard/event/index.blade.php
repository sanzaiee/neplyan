...
<div class="row page-titles">
    ...
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            ...
            <a href="{{ route('event.create') }}"> <button type="button" class="btn btn-info d-none d-lg-block m-l-15"><i
                                ...
                                <tbody>
                                   @forelse ($events as $index=>$item)
                                       <tr>
                                        <td>{{ ++$index }}</td>
                                            <td>
                                                <img src="{{ $item->image }}" alt="doc or pdf" width="100px" height="100px">
                                            </td>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->onDate }}</td>
                                            <td>{{ $item->address }}</td>
                                           ...
                                           <td>
                                                <a href="{{ route('event.edit',$item->id) }}" class="btn btn-rounded btn-xs btn-info m-r-5" data-toggle="tooltip" data-original-title="Edit">
                                                ...
                                                <form id="delete-form-{{ $item->id }}" action="{{route('event.destroy',$item->id)}}" method="post">
                                                @csrf
                                                @method('delete')
                                                </form>
                                            </td>
